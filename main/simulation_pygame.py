...
class SimulationPyGame:
    ...
    Здесь всё тоже самое, что и в классе Simulation, убрал только вывод состояния карты в консоль
    Отображение карты только через окно pygame
    """
    ...
    def create_lst_of_creatures(self):
        ...
        lst_of_beavers = self.worldmap.get_list_entities(Beaver)
        lst_of_predators = self.worldmap.get_list_entities(Predator)
        lst_of_creatures = lst_of_herbovores + lst_of_beavers + lst_of_predators
